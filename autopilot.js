...
async function checkOnDaedalusStatus (ns, player, stocksValue) {
  ...
  if (daedalusUnavailable || wdAvailable) {
    reserveForDaedalus = false
    return reserveForDaedalus
  }
  if (player.hacking < 2500) {
    reserveForDaedalus = false
    return reserveForDaedalus
  }
  ...
  if (reserveForDaedalus) { // Already waiting for a Daedalus invite, try joining them
    return (4 in unlockedSFs)
      ? await getNsDataThroughFile(ns, 'ns.joinFaction(ns.args[0])', '/Temp/joinFaction.txt', ['Daedalus'])
  ...
  if (playerInstalledAugCount !== null && playerInstalledAugCount < reqDaedalusAugs) { // Won't be able to unlock daedalus this ascend
    daedalusUnavailable = true
    return daedalusUnavailable
  }
...
async function checkIfBnIsComplete (ns, player) {
  ...
  if (player.hacking < wdHack) { return false } // We can't hack it yet, but soon!
...
async function checkOnRunningScripts (ns, player) {
  ...
  while (killScripts.length > 0) { await killScript(ns, killScripts.pop(), runningScripts) }
  ...
  if ((10 in unlockedSFs) && (2 in unlockedSFs) && !findScript('sleeve.js')) { launchScriptHelper(ns, 'sleeve.js', ['--training-reserve', 300000]) } // Only avoid training away our casino seed money
  ...
  if ((13 in unlockedSFs) && installedAugmentations.includes('Stanek\'s Gift - Genesis') && !stanekLaunched) {
    stanekLaunched = true
    if (!findScript('stanek.js')) { launchScriptHelper(ns, 'stanek.js') }
  ...
  const daemonArgs = player.hacking < hackThreshold
    ? ['--stock-manipulation']
  ...
  if ((4 in unlockedSFs) && unlockedSFs[4] < 3) { daemonArgs.push('--reserved-ram ', 32 * (unlockedSFs[4] === 2 ? 4 : 16)) }
  ...
  if (!daemon || (player.hacking >= hackThreshold && !daemon.args.includes('--looping-mode'))) {
    if (player.hacking >= hackThreshold) { log(ns, `INFO: Hack level (${player.hacking}) is >= ${hackThreshold} (--high-hack-threshold): Starting daemon.js in high-performance hacking mode.`) }
...
async function maybeDoCasino (ns, player) {
  ...
  if (player.money < 210000) { return } // We need at least 200K (and change) to run casino so we can travel to aevum
...
async function maybeInstallAugmentations (ns, player) {
  ...
  if (!shouldReset) {
    ...
    reservedPurchase = 0
    return reservedPurchase // If we were previously reserving money for a purchase, reset that flag now
  }
  ...
  if (await shouldDelayInstall(ns, player)) { // If we're currently in a state where we should not be resetting, skip reset logic
    reservedPurchase = 0
    return reservedPurchase
  } // TODO: A slick way to not have to reset this flag on every early-return statement.
  ...
  if (installCountdown > Date.now()) {
    ...
    reservedPurchase = reserveNeeded
    return reservedPurchase
  }
  ...
  } else { errLog = 'ERROR: Failed to launch ascend.js (pid == 0). Will try again later' }
  // If we got this far, something went wrong
  await persistLog(ns, errLog)
}
...
function launchScriptHelper (ns, baseScriptName, args = [], convertFileName = true) {
  ...
  if (!pid) { log(ns, `ERROR: Failed to launch ${baseScriptName} with args: [${args.join(', ')}]`, true, 'error') } else { log(ns, `INFO: Launched ${baseScriptName} (pid: ${pid}) with args: [${args.join(', ')}]`, true, 'info') }
  return pid
}
 ...
 * @param {NS} ns */
async function persistLog (ns, text) {
