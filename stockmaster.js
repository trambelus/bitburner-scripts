    ...
    while (true) {
      ...
      if (playerStats.money / corpus > fracB) {
        ...
        for (const stk of allStocks.sort(purchaseOrder)) {
          ...
          const budget = Math.min(cash, maxHoldings * (diversification + stk.spread_pct) - stk.positionValue() * (1.01 + stk.spread_pct))
...
async function initAllStocks (ns) {
  const dictMaxShares = await getStockInfoDict(ns, 'getMaxShares') // Only need to get this once, it never changes
  return allStockSymbols.map(s => ({
    ...
    expectedReturn: function () { // How much holdings are expected to appreciate (or depreciate) in the future
      ...
      const conservativeProb = normalizedProb < 0 ? Math.min(0, normalizedProb + this.probStdDev) : Math.max(0, normalizedProb - this.probStdDev)
...
async function refresh (ns, has4s, allStocks, myStocks) {
  ...
  if (ticked) {
    if (Date.now() - lastTick < expectedTickTime - sleepInterval) {
      ...
      } else { log(ns, `INFO: Detected a rapid stock market tick (${formatDuration(Date.now() - lastTick)}), likely to make up for lag / offline time.`) }
    }
    lastTick = Date.now()
  }
...
async function updateForecast (ns, allStocks, has4s) {
  ...
  for (const stk of allStocks) {
    ...
    const preNearTermWindowProb = forecast(stk.priceHistory.slice(nearTermForecastWindowLength, nearTermForecastWindowLength + marketCycleLength)) // Used to detect the probability before the potential inversion event.
  ...
  for (const stk of allStocks) {
    ...
    if (prepSummary) { // Example: AERO  ++   Prob: 54% (t51: 54%, t10: 67%) tLast⇄:190 Vol:0.640% ER: 2.778BP Spread:1.784% ttProfit: 65 Pos: 14.7M long  (held 189 ticks)
      ...
      if (stk.owned()) {
        stk.debugLog += ` Pos: ${formatNumberShort(stk.ownedShares(), 3, 1)} (${stk.ownedShares() === stk.maxShares
? 'max'
...
const updateForecastFile = async (ns, summary) => await ns.write(summaryFile, summary, 'w')
const launchSummaryTail = async ns => {
  const summaryTailScript = summaryFile.replace('.txt', '-tail.js')
  if (await getNsDataThroughFile(ns, `ns.scriptRunning('${summaryTailScript}', ns.getHostname())`, '/Temp/stockmarket-summary-is-running.txt')) { return }
...
const formatBP = fraction => formatNumberShort(fraction * 100 * 100, 3, 2) + ' BP'

/** Log / tprint / toast helper.
 * @param {NS} ns */
const log = (ns, message, tprint = false, toastStyle = '') => {
...
function doStatusUpdate (ns, stocks, myStocks, hudElement = null) {
  const maxReturnBP = 10000 * Math.max(...myStocks.map(s => s.absReturn())) // The largest return (in basis points) in our portfolio
  const minReturnBP = 10000 * Math.min(...myStocks.map(s => s.absReturn())) // The smallest return (in basis points) in our portfolio
  const estHoldingsCost = myStocks.reduce((sum, stk) => sum + (stk.owned() ? commission : 0) +
        stk.sharesLong * stk.boughtPrice + stk.sharesShort * stk.boughtPriceShort, 0)
  const liquidationValue = myStocks.reduce((sum, stk) => sum - (stk.owned() ? commission : 0) + stk.positionValue(), 0)
  const status = `Long ${myStocks.filter(s => s.sharesLong > 0).length}, Short ${myStocks.filter(s => s.sharesShort > 0).length} of ${stocks.length} stocks ` +
        (myStocks.length === 0 ? '' : `(ER ${minReturnBP.toFixed(1)}-${maxReturnBP.toFixed(1)} BP) `) +
        `Profit: ${formatMoney(totalProfit, 3)} Holdings: ${formatMoney(liquidationValue, 3)} (Cost: ${formatMoney(estHoldingsCost, 3)}) ` +
        `Net: ${formatMoney(totalProfit + liquidationValue - estHoldingsCost, 3)}`
  log(ns, status)
...
function initializeHud () {
  ...
  stockValueTracker.querySelectorAll('p').forEach((el, i) => {
    el.id = 'stock-display-' + i
    return el.id
  })
