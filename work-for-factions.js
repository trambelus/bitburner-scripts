...
export async function main (ns) {
  ...
  if (options['crime-focus'] && options['no-focus']) { return log(ns, 'ERROR: Cannot use --no-focus and --crime-focus at the same time. You need to focus to do crime!', true, 'error') }
  ...
  if (!(4 in dictSourceFiles)) { return log(ns, 'ERROR: You cannot automate working for factions until you have unlocked singularity access (SF4).', true, 'error') } else if (dictSourceFiles[4] < 3) {
...
async function loadStartupData (ns) {
  ...
  if (playerGang && !options['disable-treating-gang-as-sole-provider-of-its-augs']) {
    ...
    for (const faction of allKnownFactions.filter(f => f !== playerGang)) { dictFactionAugs[faction] = dictFactionAugs[faction].filter(a => !gangAugs.includes(a)) }
  }
  ...
  if (completedFactions.length > 0) { ns.print(`${completedFactions.length} factions will be skipped (for having all augs purchased): ${completedFactions.join(', ')}`) }
  if (softCompletedFactions.length > 0) { ns.print(`${softCompletedFactions.length} factions will initially be skipped (all desired augs purchased): ${softCompletedFactions.join(', ')}`) }
...
async function mainLoop (ns) {
  ...
  if (2 in dictSourceFiles) {
    ...
    if (ns.heart.break() <= options['karma-threshold-for-gang-invites']) { // Start trying to earn gang faction invites if we're close to unlocking gangs
      if (!playerGang) {
        ...
        for (const factionName of desiredGangFactions) { await earnFactionInvite(ns, factionName) }
      }
    }
  }
  ...
  for (const faction of factionWorkOrder) { if (!breakToMainLoop()) await workForSingleFaction(ns, faction) }
  ...
  const megacorpFactions = preferredCompanyFactionOrder.filter(f => !skipFactions.includes(f))
  ...
  const joinedFactions = player.factions // In case our hard-coded list of factions is missing anything, merge it with the list of all factions
  const knownFactions = factions.concat(joinedFactions.filter(f => !factions.includes(f)))
  const allIncompleteFactions = knownFactions.filter(f => !skipFactions.includes(f) && !completedFactions.includes(f)).sort((a, b) => mostExpensiveAugByFaction[a] - mostExpensiveAugByFaction[b])
  ...
  for (const faction of allIncompleteFactions.filter(f => !softCompletedFactions.includes(f))) { if (!breakToMainLoop()) await workForSingleFaction(ns, faction) }
  ...
  const factionsWeCanWorkFor = joinedFactions.filter(f => !options.skip.includes(f) && !cannotWorkForFactions.includes(f) && f !== playerGang)
  let foundWork = false
  if (factionsWeCanWorkFor.length > 0 && !options['crime-focus']) {
    ...
    const mostFavorFaction = factionsWeCanWorkFor.sort((a, b) => dictFactionFavors[b] - dictFactionFavors[a])
      .filter(faction => faction !== 'Shadows of Anarchy')[0]
    const targetRep = 1000 + (await getFactionReputation(ns, mostFavorFaction)) * 1.05 // Hack: Grow rep by ~5%, plus 1000 incase it's currently 0
...
async function earnFactionInvite (ns, factionName) {
  ...
  const reasonPrefix = `Cannot join faction "${factionName}" because`
  ...
  if (requirement && player.hacking < requirement &&
    ...
    if (options['no-studying']) { return ns.print('--no-studying is set, nothing we can do to improve hack level.') } else if (heuristic < em) {
    ...
    if (studying) { workedForInvite = await monitorStudies(ns, 'hacking', requirement) }
  ...
  if ((requirement = requiredMoneyByFaction[factionName]) && player.money < requirement) { return ns.print(`${reasonPrefix} you have insufficient money. Need: ${formatMoney(requirement)}, Have: ${formatMoney(player.money)}`) }

  // If travelling can help us join a faction - we can do that too
  player = await getPlayerInfo(ns)
  let travelledForInvite = false
  const travelToCityOrDidRecently = async city => // Helper to consider us as having travelled for an invite if we did just now, or recently
    (player.city !== city && await goToCity(ns, city)) || (player.city === city && (Date.now() - lastTravel < 60000))
  if (['Tian Di Hui', 'Tetrads', 'The Dark Army'].includes(factionName)) { travelledForInvite = await travelToCityOrDidRecently('Chongqing') } else if (['The Syndicate'].includes(factionName)) { travelledForInvite = await travelToCityOrDidRecently('Sector-12') } else if (['Aevum', 'Chongqing', 'Sector-12', 'New Tokyo', 'Ishima', 'Volhaven'].includes(factionName)) { travelledForInvite = await travelToCityOrDidRecently(factionName) }
  ...
  if (factionName === 'Silhouette') {
    ...
    const factionConfig = companySpecificConfigs.find(f => f.name === factionName) // We set up Silhouette with a "company-specific-config" so that we can work for an invite like any megacorporation faction.
    const companyNames = preferredCompanyFactionOrder.map(f => companySpecificConfigs.find(cf => cf.name === f)?.companyName || f)
    const favorByCompany = await getNsDataThroughFile(ns, dictCommand('ns.getCompanyFavor(o)'), '/Temp/getCompanyFavors.txt', companyNames)
    const repByCompany = await getNsDataThroughFile(ns, dictCommand('ns.getCompanyRep(o)'), '/Temp/getCompanyReps.txt', companyNames)
...
async function goToCity (ns, cityName) {
  ...
  if (player.money < 200000) { announce(ns, `WARN: Insufficient funds to travel from ${player.city} to ${cityName}`, 'warning') } else { announce(ns, `ERROR: Failed to travel from ${player.city} to ${cityName} for some reason...`, 'error') }
  return false
}
  ...
  while (forever || player.strength < reqStats || player.defense < reqStats || player.dexterity < reqStats || player.agility < reqStats || player.numPeopleKilled < reqKills || -ns.heart.break() < reqKarma) {
    ...
    crime = crimeCount < 10
      ...
      : (!needStats && (player.numPeopleKilled < reqKills || karma < reqKarma))
          ? 'homicide' // If *all* we need now is kills or Karma, homicide is the fastest way to do that, even at low proababilities
...
async function study (ns, focus, course, university = null) {
  if (options['no-studying']) { return announce(ns, `WARNING: Could not study '${course}' because --no-studying is set.`, 'warning') }
  ...
  if (!university) { // Auto-detect the university in our city
    ...
    if (!university) { return announce(ns, `WARNING: Could not study '${course}' because we are in city '${playerCity}' without a university.`, 'warning') }
  }
...
async function monitorStudies (ns, stat, requirement) {
  ...
  while (!breakToMainLoop()) {
    const player = await getPlayerInfo(ns)
    if (!player.className) { return announce(ns, 'WARNING: Somebody interrupted our studies.', 'warning') }
...
export async function tryJoinFaction (ns, factionName) {
  const joinedFactions = (await getPlayerInfo(ns)).factions
  if (joinedFactions.includes(factionName)) { return true }
  if (!(await getNsDataThroughFile(ns, 'ns.joinFaction(ns.args[0])', '/Temp/join-faction.txt', [factionName]))) { return false }
...
async function getFactionReputation (ns, factionName) {
  if (factionName === undefined) return 0
...
export async function workForSingleFaction (ns, factionName, forceUnlockDonations = false, forceBestAug = false, forceRep = undefined) {
  ...
  let factionRepRequired = forceRep || (forceUnlockDonations ? favorRepRequired : Math.min(highestRepAug, favorRepRequired))
  if (highestRepAug === -1 && !firstFactions.includes(factionName) && !forceRep && !options['get-invited-to-every-faction']) { return ns.print(`All "${factionName}" augmentations are owned. Skipping unlocking faction...`) }
  // Ensure we get an invite to location-based factions we might want / need
  if (!await earnFactionInvite(ns, factionName)) { return ns.print(`We are not yet part of faction "${factionName}". Skipping working for faction...`) }
  ...
  if (options['prioritize-invites'] && !forceUnlockDonations && !forceBestAug && !forceRep) { return ns.print('--prioritize-invites Skipping working for faction for now...') }

  let lastStatusUpdateTime = 0; let repGainRatePerMs = 0
  ...
  while ((currentReputation = (await getFactionReputation(ns, factionName))) < factionRepRequired) {
    ...
    if (factionWork === undefined) continue
    ...
    const status = `Doing '${factionWork}' work for "${factionName}" until ${Math.round(factionRepRequired).toLocaleString()} rep.`
    if (lastFactionWorkStatus !== status || (Date.now() - lastStatusUpdateTime) > statusUpdateInterval) {
      ...
      const measuredRepGainRatePerMs = ((await getFactionReputation(ns, factionName)) - lastRepMeasurement) / (Date.now() - lastStatusUpdateTime)
      if (currentReputation > lastRepMeasurement + statusUpdateInterval * repGainRatePerMs * 2) { // Detect a sudden increase in rep, but don't use it to update the expected rate
        ...
        repGainRatePerMs = measuredRepGainRatePerMs
      } // If we measure a significantly different rep gain rate, this could be due to external sources of rep (e.g. sleeves) - account for it in the ETA
      lastStatusUpdateTime = Date.now(); lastRepMeasurement = currentReputation
      const etaMilliseconds = (factionRepRequired - currentReputation) / repGainRatePerMs
      ns.print((lastFactionWorkStatus = status) + ` Currently at ${Math.round(currentReputation).toLocaleString()}, earning ${formatNumberShort(repGainRatePerMs * 1000)} rep/sec. ` +
                (hasFocusPenaly && !shouldFocusAtWork ? 'after 20% non-focus Penalty ' : '') + `(ETA: ${formatDuration(etaMilliseconds)})`)
    }
    ...
    if (!forceBestAug && !forceRep) { // Detect our rep requirement decreasing (e.g. if we exported for our daily +1 faction rep)
      const currentFavor = await getCurrentFactionFavour(ns, factionName)
    ...
    const workRepGained = (await getPlayerInfo(ns)).workRepGained // Delay the next loop slightly until the next game tick so we aren't missing out on a few ms of rep
    ...
    if (currentReputation + (await getPlayerInfo(ns)).workRepGained >= factionRepRequired) { await stop(ns) } // We're close - stop working so our current rep is accurate when we check the while loop condition
  }
  if (currentReputation >= factionRepRequired) { ns.print(`Attained ${Math.round(currentReputation).toLocaleString()} rep with "${factionName}" (needed ${factionRepRequired.toLocaleString()}).`) }
  if (factionName === 'Daedalus') await daedalusSpecialCheck(ns, favorRepRequired, currentReputation)
...
async function detectBestFactionWork (ns, factionName) {
  let bestWork; let bestRepRate = 0
...
export async function workForAllMegacorps (ns, megacorpFactionsInPreferredOrder, alsoWorkForCompanyFactions, oneCompanyFactionAtATime) {
  const player = (await getPlayerInfo(ns))
  if (player.hacking < 225) { return ns.print(`Hacking Skill ${player.hacking} is to low to work for any megacorps (min req. 225).`) }
  const joinedCompanyFactions = player.factions.filter(f => megacorpFactionsInPreferredOrder.includes(f)) // Company factions we've already joined
  if (joinedCompanyFactions.length > 0) { ns.print(`${joinedCompanyFactions.length} companies' factions have already been joined: ${joinedCompanyFactions.join(', ')}`) }
  const doFactionWork = alsoWorkForCompanyFactions && oneCompanyFactionAtATime
  ...
  for (const factionName of megacorpFactionsInPreferredOrder) {
    ...
    if (earnedInvite && doFactionWork && !breakToMainLoop()) { await workForSingleFaction(ns, factionName) }
  ...
  if (alsoWorkForCompanyFactions && !oneCompanyFactionAtATime) { // If configured, start grinding rep with company factions we've joined
    ...
    for (const factionName of megacorpFactionsInPreferredOrder) { if (!breakToMainLoop()) await workForSingleFaction(ns, factionName) }
  }
}
...
export async function tryBuyReputation (ns) {
  ...
  if ((await getPlayerInfo(ns)).money > 100E9) { // If we're wealthy, hashes have relatively little monetary value, spend hacknet-node hashes on contracts to gain rep faster
    const spentHashes = await trySpendHashes(ns, 'Generate Coding Contract')
    if (spentHashes > 0) {
...
export async function workForMegacorpFactionInvite (ns, factionName, waitForInvite) {
  ...
  if ((await checkFactionInvites(ns)).includes(factionName)) { return waitForInvite ? await waitForFactionInvite(ns, factionName) : false }
  ...
  let currentReputation; let currentRole = ''; let currentJobTier = -1 // TODO: Derive our current position and promotion index based on player.jobs[companyName]
  let lastStatus = ''; let lastStatusUpdateTime = 0; let repGainRatePerMs = 0
  let lastRepMeasurement = await getCompanyReputation(ns, companyName)
  let studying = false; let working = false; let backdoored = false
  while (((currentReputation = (await getCompanyReputation(ns, companyName))) < repRequiredForFaction) && !player.factions.includes(factionName)) {
    ...
    const qualifyingItTier = getTier(itJob); const qualifyingSoftwareTier = getTier(softwareJob)
    ...
    if (currentReputation >= requiredRep && player.hacking >= requiredHack && player.charisma < requiredCha && !options['no-studying']) {
      ...
      if (!studying) { // Study at ZB university if CHA is the limiter.
        if (await studyForCharisma(ns, shouldFocusAtWork)) { working = !(studying = true) }
      }
      if (requiredCha - player.charisma > 10) { // Try to spend hacknet-node hashes on university upgrades while we've got a ways to study to make it go faster
        const spentHashes = await trySpendHashes(ns, 'Improve Studying')
        if (spentHashes > 0) {
    ...
    if (await tryApplyToCompany(ns, companyName, currentRole)) { announce(ns, `Unexpected '${currentRole}' promotion from ${currentJob} to "${(await getPlayerInfo(ns)).jobs[companyName]}. Promotion logic must be off..."`, 'warning') }
    ...
    if (lastStatus !== status || (Date.now() - lastStatusUpdateTime) > statusUpdateInterval) {
      ...
      // Actually measure how much reputation we've earned since our last update, to give a more accurate ETA including external sources of rep
      const measuredRepGainRatePerMs = ((await getCompanyReputation(ns, companyName)) - lastRepMeasurement) / (Date.now() - lastStatusUpdateTime)
      if (currentReputation > lastRepMeasurement + statusUpdateInterval * repGainRatePerMs * 2) { // Detect a sudden increase in rep, but don't use it to update the expected rate
        ns.print('SUCCESS: Reputation spike! (Perhaps a coding contract was just solved?) ETA reduced.')
      } else if (lastStatusUpdateTime !== 0 && Math.abs(measuredRepGainRatePerMs - repGainRatePerMs) / repGainRatePerMs > 0.05) { // Stick to the game-provided rate if we measured something within 5% of that number
      ...
      lastStatusUpdateTime = Date.now(); lastRepMeasurement = currentReputation
      const etaMilliseconds = ((requiredRep || repRequiredForFaction) - currentReputation) / repGainRatePerMs
      player = (await getPlayerInfo(ns))
      ...
      ns.print(`Currently a "${player.jobs[companyName]}" ('${currentRole}' #${currentJobTier}) for "${companyName}" earning ${formatNumberShort(repGainRatePerMs * 1000)} rep/sec. ` +
                ...
                `Rep:${Math.round(currentReputation).toLocaleString()} ${currentReputation >= (requiredRep || repRequiredForFaction) ? '✓' : `✗ (ETA: ${formatDuration(etaMilliseconds)})`}`)
      lastStatus = status
    }
