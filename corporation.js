/**
 * From https://github.com/malaclypse2/bitburner-scripts-1/tree/corporation-managment
...
class Industry {
  constructor (name = '', robFac = 0.0, aiFac = 0.0, advFac = 0.0, sciFac = 0.0, hwFac = 0.0, reFac = 0.0, reqMats = {}, prodMats = [], makesProducts = false, startupCost = 0) {
    ...
    const scaleFactor = Object.values(this.materialBonusPerSqMeter).reduce((sum, prod) => sum + prod, 0)
...
export async function main (ns) {
  ...
  const runOnce = options.o || options.once
  const shouldManage = !options['price-discovery-only']
...
async function doManageCorporation (ns) {
  ...
  myCorporation = ns.corporation.getCorporation()
  const netIncome = myCorporation.revenue - myCorporation.expenses
  const now = new Date().toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true })
  ...
  for (const unlockable of availableUnlocks) {
    const cost = ns.corporation.getUnlockUpgradeCost(unlockable)
  ...
  for (const upgrade of upgrades) {
    ...
    const nextLevel = ns.corporation.getUpgradeLevel(upgrade) + 1
    if (cost > budget) continue
    if (upgrade === 'Wilson Analytics' && cost < budget * 0.9 && hasProductionDivision) {
      ...
      const priority = Math.round((1 - cost / budget) * 100)
  ...
  if (myCorporation.divisions.length === 0) {
    ...
    const newDivisionBudget = budget * 0.9
    ...
    const newIndustry = possibleIndustries.find((ind) => ind.name === options.first)
    if (newIndustry) {
  ...
  if (myCorporation.divisions.length >= desiredDivisions) {
    if (options['can-spend-hashes']) { await doSpendHashes(ns, 'Exchange for Corporation Research') }
  }
  ...
  /**
      ...
      */
  const spent = await runTasks(ns, tasks, budget)
  ...
  for (const division of myCorporation.divisions) {
    ...
    const spent = await doManageDivision(ns, division, divisionalBudget)
    if (spent) budget -= spent
  }
}
...
async function tryRaiseCapital (ns) {
  ...
  if (options['can-spend-hashes'] && myCorporation.funds < 10e9) { await doSpendHashes(ns, 'Sell for Corporation Funds') }
    ...
    if (offer && offer.round <= 4) {
      ...
      for (const division of myCorporation.divisions) {
        ...
        for (const city of division.cities) {
          ...
          const warehouseSpaceRequiredForCycle = getReservedWarehouseSpace(ns, industry, division, city)
          const warehouseSpaceAvailable = warehouse.size - warehouseSpaceRequiredForCycle - warehouse.sizeUsed
      ...
      for (const division of myCorporation.divisions) {
        ...
        if (offer.round >= 4 && industry.makesProducts) {
          ...
          const completeProducts = division.products.map((prodName) => ns.corporation.getProduct(division.name, prodName)).filter((prod) => prod.developmentProgress >= 100)
...
function allEmployeesSatisfied (ns, lowerLimit = 0.9995) {
  ...
  for (const division of myCorporation.divisions) {
    for (const city of division.cities) {
      ...
      const employees = office.employees.map((e) => ns.corporation.getEmployee(division.name, city, e))
      const avgMorale = employees.map((e) => e.mor).reduce((sum, mor) => sum + mor, 0) / employees.length
      const avgEnergy = employees.map((e) => e.ene).reduce((sum, ene) => sum + ene, 0) / employees.length
      const avgHappiness = employees.map((e) => e.hap).reduce((sum, hap) => sum + hap, 0) / employees.length
...
async function doCreateNewDivision (ns, newIndustry, newDivisionBudget) {
  ...
  myCorporation = ns.corporation.getCorporation()
  const numDivisions = myCorporation.divisions.length
  ...
  const newDivision = ns.corporation.getDivision(newIndustry.name)
...
async function doManageDivision (ns, division, budget) {
  ...
  const hasMarketTA2 = ns.corporation.hasResearched(division.name, 'Market-TA.II')
  ...
  const adPrice = ns.corporation.getHireAdVertCost(division.name)
  ...
  const researchTypes = ['Hi-Tech R&D Laboratory', 'uPgrade: Fulcrum', 'uPgrade: Capacity.I', 'uPgrade: Capacity.II', 'Market-TA.I', 'Market-TA.II']
  ...
  if (industry.makesProducts) {
    const maxProducts = getMaxProducts(ns, division.name)
    const products = division.products.map((p) => ns.corporation.getProduct(division.name, p))
    ...
    if (progress === 100) {
      ...
      if (products.length < maxProducts) {
        ...
        budget -= spent
      } else { // Discontinue an existing product for a new one if we're not raising capital.
        ...
        if (raisingCapital === 0) {
          if (extraReserve > 0 && myCorporation.funds > extraReserve) {
            ...
            const lowBudgetProduct = products[0]
  ...
  for (const city of division.cities) {
    ...
    if (ns.corporation.hasUnlockUpgrade('Smart Supply') && !warehouse.smartSupplyEnabled) {
      ...
      for (const requiredMaterialName in industry.reqMats) {
        const amtPerProduct = industry.reqMats[requiredMaterialName]
        let amtRequiredMaterial = 0
        for (const producedMaterialName of industry.prodMats) {
          const producedMaterial = ns.corporation.getMaterial(division.name, city, producedMaterialName)
    ...
    const warehouseSpaceAvailable = freeSpace - warehouseSpaceRequiredForCycle
    const tolerance = warehouseSpaceRequiredForCycle * 0.01
    const enoughSpace = warehouseSpaceAvailable >= tolerance // Tiny safety margin
    const satisfied = allEmployeesSatisfied(ns)
    ...
    if (warehouseSpaceAvailable < -tolerance) {
      ...
      for (const material of allMaterials) {
        const amt = ns.corporation.getMaterial(division.name, city, material).qty
        const sellAmt = amt * 0.025
...
function getReservedWarehouseSpace (ns, industry, division, city) {
  ...
  for (const matName in industry.reqMats) {
    const matAmt = industry.reqMats[matName]
...
function getMaximumProduction (ns, division, city) {
  const office = ns.corporation.getOffice(division.name, city)
  const officeMult = getOfficeProductivity(office) // Workers
  const prodMult = division.prodMult // Materials
  const corpMult = 1 + 0.03 * ns.corporation.getUpgradeLevel('Smart Factories') // Corporate upgrades.
  let resMult = 1
  ...
  const maxProd = officeMult * prodMult * corpMult * resMult
  return maxProd
}
...
function createNewProduct (ns, division) {
  ...
  const productname = `${division.type}-${Math.log10(wantToSpend).toFixed(2)}`
  try {
...
async function fillOpenPositionsFromQueue (ns) {
  ...
  while (['START'].includes(myCorporation.state) && fillSpaceQueue.length > 0) {
    const office = fillSpaceQueue.shift()
    const divisionName = office.split('/')[0]
    const cityName = office.split('/')[1]
...
async function fillOpenPositions (ns, divisionName, cityName) {
  ...
  const employees = office.employees.map((e) => ns.corporation.getEmployee(divisionName, cityName, e))
  const numUnassigned = employees.filter((e) => e.pos === 'Unassigned').length
  ...
  if (openJobs > 0) {
    ...
    const employeesPerJob = Math.floor(office.employees.length / jobs.length)
    const employeesLeft = office.employees.length % jobs.length
...
async function doPriceDiscovery (ns) {
  ...
  for (const division of myCorporation.divisions) {
    ...
    for (const materialName of industry.prodMats) {
      for (const city of division.cities) {
        try {
          ...
          ns.print(`WARN: could not establish price for ${city}: ${err.toString()}`)
          continue
        }
      }
    }
    ...
    for (const productName of division.products) {
      ...
      try {
        const sMult = sPrice.split('*')[1]
        lastPriceMultiplier = Number.parseFloat(sMult)
      } catch {}
      const votes = []
      for (const city of division.cities) {
        // Each city is going to "vote" for how they want the price to be manipulated.
        const qty = product.cityData[city][0]
        const produced = product.cityData[city][1]
        const sold = product.cityData[city][2]
      ...
      votes.sort((a, b) => a - b)
      const newMultiplier = votes[0]
      const newPrice = `MP*${newMultiplier.toFixed(3)}`
      // if (verbose) log(ns, `${prefix}Votes: ${votes.map((n) => nf(n)).join(', ')}.`);
      const sChange = percentChange(lastPriceMultiplier, newMultiplier)
...
function budgetFromProductName (projectName) {
  const sExp = projectName.split('-')[1]
  const exp = Number.parseFloat(sExp)
...
function percentChange (oldVal, newVal) {
  const percentChange = (newVal / oldVal) * 100 - 100
